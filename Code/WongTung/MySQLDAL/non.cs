...
namespace WongTung.MySQLDAL
{
	...
	public class non:Inon
	{
		...
		public WongTung.Model.non GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select CO_CODE,STAFF_CODE,DATE,TYPE,ANNUAL,SICK,ADMIN,OT_PAY from non ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
};

			WongTung.Model.non model=new WongTung.Model.non();
