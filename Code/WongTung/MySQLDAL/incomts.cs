...
namespace WongTung.MySQLDAL
{
	...
	public class incomts:Iincomts
	{
		...
		public void Add(WongTung.Model.incomts model)
		{
			...
			strSql.Append("insert into incomts(");
			strSql.Append("IST_CO_CODE,IST_OFFCIE_CODE,IST_WORK_DATE,IST_USER_CODE,IST_USER_NAME,IST_INPUT_OK,IST_APP,IST_NOR_HR,IST_OT_HR,IST_PERIOD)");
			strSql.Append(" values (");
			strSql.Append("@IST_CO_CODE,@IST_OFFCIE_CODE,@IST_WORK_DATE,@IST_USER_CODE,@IST_USER_NAME,@IST_INPUT_OK,@IST_APP,@IST_NOR_HR,@IST_OT_HR,@IST_PERIOD)");
			MySqlParameter[] parameters = {
					new MySqlParameter("@IST_CO_CODE", MySqlDbType.Char,3),
					new MySqlParameter("@IST_OFFCIE_CODE", MySqlDbType.Char,3),
			...
			parameters[1].Value = model.IST_OFFCIE_CODE;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update incomts set ");
			strSql.Append("IST_CO_CODE=@IST_CO_CODE,");
			strSql.Append("IST_OFFCIE_CODE=@IST_OFFCIE_CODE,");
		...
		public WongTung.Model.incomts GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select IST_CO_CODE,IST_OFFCIE_CODE,IST_WORK_DATE,IST_USER_CODE,IST_USER_NAME,IST_INPUT_OK,IST_APP,IST_NOR_HR,IST_OT_HR,IST_PERIOD from incomts ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
};
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				model.IST_OFFCIE_CODE=ds.Tables[0].Rows[0]["IST_OFFCIE_CODE"].ToString();
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select IST_CO_CODE,IST_OFFCIE_CODE,IST_WORK_DATE,IST_USER_CODE,IST_USER_NAME,IST_INPUT_OK,IST_APP,IST_NOR_HR,IST_OT_HR,IST_PERIOD ");
			strSql.Append(" FROM incomts ");
			if(strWhere.Trim()!="")
			{
